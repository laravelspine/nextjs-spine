...
import { useModuleExtensions } from "./module-extensions";
import {
  useDashboardState,
...
function AreaColumn({
  ...
  return (
    <div className="flex flex-col">
      <p className="px-1 pb-1.5 text-xs font-medium uppercase tracking-wide text-ink-faint">
        {areaLabel(area)}
      </p>
      <div
        ref={ref}
        ...
        className={cx(
          ...
          // Saat drag aktif, SEMUA area terlihat sebagai dropzone.
          dragging && "border-dashed border-line-soft",
          isDropTarget && "border-solid border-accent/40 bg-accent-soft/20"
        )}
      >
        {isPreview ? (
          <div className="flex min-h-32 flex-1 items-center justify-center rounded-xl border-2 border-dashed border-line-soft px-4 py-6 text-center text-xs text-ink-faint">
...
export function DashboardGrid() {
  ...
  const widgets: ModuleWidget[] = ext.widgets ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-line-soft bg-surface-raised px-4 py-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <details className="group relative">
            <summary className="cursor-pointer list-none rounded-lg border border-line-soft bg-surface-raised px-3 py-1.5 text-xs font-medium text-ink-muted transition-colors hover:bg-surface-overlay [&::-webkit-details-marker]:hidden">
              Widgets
              <span className="ml-1.5 text-ink-faint">
                ({widgets.filter((w: ModuleWidget) => isWidgetVisible(w.id, visibility)).length}/{widgets.length})
              </span>
            </summary>
            <div className="absolute right-0 z-20 mt-2 w-60 rounded-xl border border-line-soft bg-surface-raised p-3 shadow-lg">
              <ul className="space-y-1.5">
                {widgets.map((w: ModuleWidget) => {
          ...
          <button
            ...
            className={cx(
              ...
              preview
                ...
                : "border-line-soft bg-surface-raised text-ink-muted hover:bg-surface-overlay"
            )}
          >
          ...
          <button
            type="button"
            onClick={handleReset}
            className="rounded-lg border border-line-soft bg-surface-raised px-3 py-1.5 text-xs font-medium text-ink-muted transition-colors hover:bg-surface-overlay"
          >
            Reset dashboard
          </button>
        </div>
      ...
      <DragDropProvider onDragStart={onDragStart} onDragEnd={onDragEnd}>
        ...
        {dragLabel && (
          <DragOverlay>
            <div className="rounded-xl border border-line-soft bg-surface-raised px-4 py-3 text-sm font-semibold text-ink shadow-lg">
              {dragLabel}
            </div>
          </DragOverlay>
        )}
